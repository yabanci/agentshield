...
func TestLatency_EmptyReturnsZero(t *testing.T) {
	lt := agent.NewTestLatencyTracker()
	if got := lt.P95(agent.TierPrimary); got != 0 {
...
func TestLatency_SingleSample(t *testing.T) {
	lt := agent.NewTestLatencyTracker()
	lt.Record(agent.TierPrimary, 100*time.Millisecond)
	if got := lt.P95(agent.TierPrimary); got != 100 {
...
func TestLatency_P95IsCorrect(t *testing.T) {
	...
	// p95 = 95th rank = 95ms.
	for i := 1; i <= 100; i++ {
		lt.Record(agent.TierPrimary, time.Duration(i)*time.Millisecond)
	}
	got := lt.P95(agent.TierPrimary)
...
func TestLatency_RingBufferWrap(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		lt.Record(agent.TierPrimary, 500*time.Millisecond)
	}
	if got := lt.P95(agent.TierPrimary); got != 500 {
...
func TestLatency_PerTierIsolated(t *testing.T) {
	lt := agent.NewTestLatencyTracker()
	lt.Record(agent.TierPrimary, 1*time.Second)
	lt.Record(agent.TierFallback, 100*time.Millisecond)
	lt.Record(agent.TierCache, 1*time.Millisecond)

	if lt.P95(agent.TierPrimary) != 1000 {
		t.Errorf("primary p95 wrong: %d", lt.P95(agent.TierPrimary))
	}
	if lt.P95(agent.TierFallback) != 100 {
		t.Errorf("fallback p95 wrong: %d", lt.P95(agent.TierFallback))
	}
	if lt.P95(agent.TierCache) != 1 {
		t.Errorf("cache p95 wrong: %d", lt.P95(agent.TierCache))
	}
}
...
func TestLatency_ConcurrentRecords(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		go func(d int) {
			defer wg.Done()
			lt.Record(agent.TierPrimary, time.Duration(d)*time.Millisecond)
		}(i + 1)
	}
	wg.Wait()
	if got := lt.P95(agent.TierPrimary); got <= 0 {
...
func TestLatency_Snapshot(t *testing.T) {
	lt := agent.NewTestLatencyTracker()
	lt.Record(agent.TierPrimary, 100*time.Millisecond)
	lt.Record(agent.TierFallback, 200*time.Millisecond)
